...
- **THEN** 文档列表不包含它；其它非下划线子目录（如 `notes/`）中的文档仍正常收录
...
#### Scenario: 子目录文档中的图片
- **WHEN** 文档位于 `management/docs/notes/<slug>.md` 并引用 `/api/management/docs-assets/<slug>/fig-1.webp`
- **THEN** 页面在 `/management/docs/notes/<slug>` 路由下能正常加载该图片
