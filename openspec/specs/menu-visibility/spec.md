...
通过集中配置的 hidden key 列表控制侧边栏菜单项与报告类型 tab 的显示/隐藏。隐藏只影响入口可见性，不删除路由，配置随时可逆。

## Requirements

### Requirement: 集中配置的隐藏列表

系统 SHALL 提供一个集中的前端常量配置（`HIDDEN_KEYS` 列表），作为菜单与报告类型 tab 可见性的唯一控制点；列表为空时所有功能 SHALL 全部显示（与现状一致）。

#### Scenario: 配置为空
- **WHEN** `HIDDEN_KEYS` 为空数组
- **THEN** 侧边栏显示全部分组与菜单项，报告页显示全部三个类型 tab，行为与无此功能时完全一致

### Requirement: 菜单项级隐藏

`HIDDEN_KEYS` 中的叶子 key（路由 path，如 `/management/reports`）命中时，对应菜单项 SHALL 不显示，但其路由 SHALL 保持注册——直接输入 URL 仍可访问该页面。
...
`HIDDEN_KEYS` 中的报告类型 key（`reports:daily` / `reports:weekly` / `reports:monthly`）命中时，报告页对应的类型 tab SHALL 不显示；被隐藏类型的数据与路由 SHALL 不受影响。当前激活类型被隐藏时，页面 SHALL 自动切换到第一个可见类型；全部类型被隐藏属配置错误，此时报告菜单项（若可见）点击 SHALL 仍进入页面并显示空态提示。
